...
class TestGoogleCloudCredentialsManager:
  """Test suite for GoogleCloudCredentialsManager OAuth flow handling.
  ...
  def manager(self, credentials_config):
    ...
    return GoogleCloudCredentialsManager(credentials_config)
  ...
  async def test_oauth_flow_completion_with_caching(
    ...
    with patch(
        "google.adk.tools.google_cloud_credentials.google.oauth2.credentials.Credentials",
  ...
  async def test_cache_persistence_across_manager_instances(
    ...
    manager1 = GoogleCloudCredentialsManager(credentials_config)
    ...
    manager2 = GoogleCloudCredentialsManager(credentials_config)
    ...
    with patch(
        "google.adk.tools.google_cloud_credentials.google.oauth2.credentials.Credentials.from_authorized_user_info"
    ) as mock_from_json:
      mock_cached_creds = Mock(spec=OAuthCredentials)
      mock_cached_creds.valid = True
      mock_from_json.return_value = mock_cached_creds

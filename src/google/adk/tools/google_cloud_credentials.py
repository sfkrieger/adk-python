...
@experimental
class GoogleCloudCredentialsConfig(BaseModel):
  ...
  """The default scopes to use if not provided."""
  token_cache_key: Optional[str] = None
  """The key to cache the token in the tool context."""

  @model_validator(mode="after")
  def __post_init__(self) -> GoogleCloudCredentialsConfig:
    ...
    self.token_cache_key = (
        self.token_cache_key
        if self.token_cache_key
        else "credentials_token_cache"
    )

    return self


class GoogleCloudCredentialsManager:
  ...
  def __init__(
      self,
      credentials_config: GoogleCloudCredentialsConfig,
  ):
  ...
  async def get_valid_credentials(
    ...
    creds_json = tool_context.state.get(
        self.credentials_config.token_cache_key, None
    )
    creds = (
    ...
    if creds and creds.expired and creds.refresh_token:
      try:
        ...
        if creds.valid:
          ...
          tool_context.state[self.credentials_config.token_cache_key] = (
              creds.to_json()
          )
          return creds
      except RefreshError:
